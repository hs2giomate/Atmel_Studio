...
void CAN_MAX_tx_callback(struct can_async_descriptor *const descr)
{
	(void)descr;
}
void CAN_MAX_rx_callback(struct can_async_descriptor *const descr)
{
...
/**
 * Example of using CAN_MAX to Encrypt/Decrypt datas.
 */
void CAN_MAX_example(void)
{
	struct can_message msg;
	...
	can_async_register_callback(&CAN_MAX, CAN_ASYNC_TX_CB, (FUNC_PTR)CAN_MAX_tx_callback);
	can_async_enable(&CAN_MAX);

	/**
	 * CAN_MAX_tx_callback callback should be invoked after call
	 * can_async_write, and remote device should recieve message with ID=0x45A
	 */
	can_async_write(&CAN_MAX, &msg);
	...
	/**
	 * CAN_MAX_rx_callback callback should be invoked after call
	 * can_async_set_filter and remote device send CAN Message with the same
	 * content as the filter.
	 */
	can_async_register_callback(&CAN_MAX, CAN_ASYNC_RX_CB, (FUNC_PTR)CAN_MAX_rx_callback);
	filter.id   = 0x469;
	filter.mask = 0;
	can_async_set_filter(&CAN_MAX, 0, CAN_FMT_STDID, &filter);

	filter.id   = 0x10000096;
	filter.mask = 0;
	can_async_set_filter(&CAN_MAX, 1, CAN_FMT_EXTID, &filter);
}
