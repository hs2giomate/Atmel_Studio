...
//	Write an array of "len" uint8_ts ("buffer"), starting at register "reg", and
...
uint8_t MMA8452Q::readRegister(MMA8452Q_Register reg)
{
	...
	//io_write(I2C_io, &memReg, sizeof(memReg));
	//Wire.write(reg);
...
void MMA8452Q::readRegisters(MMA8452Q_Register reg, uint8_t *buffer, uint8_t len)
{
	...
	//Wire.endTransmission(false); //endTransmission but keep the connection active
/*
	Wire.requestFrom(addressI2C, len); //Ask for uint8_ts, once done, bus is released by default
	
	i2c_m_async_cmd_read(&I2C_0, uint8_t reg, uint8_t *value)
