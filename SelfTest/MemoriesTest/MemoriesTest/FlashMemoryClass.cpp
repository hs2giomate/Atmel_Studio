...
void FlashMemoryClass::flushConfigurationData(userParameters *data)
{
	...
	writeFlash((uint32_t)&memoryAddress->parameters, &appData, sizeof(userParameters));
	flashClose();
...
void FlashMemoryClass::readConfigurationData(userParameters *data)
{
	...
	flashOpen();
	readFlash(&appData, (uint32_t)&memoryAddress->parameters, sizeof(userParameters));
	flashClose();
