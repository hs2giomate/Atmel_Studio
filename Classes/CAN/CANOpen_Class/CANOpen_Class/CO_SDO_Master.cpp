...
static void CO_SDOclient_receive(void *object, const CO_CANrxMsg_t *msg){
    ...
    if((msg->DLC == 8U) && (!SDO_C->CANrxNew) && (SDO_C->state != SDO_STATE_NOTDEFINED)){
        ...
        if(SDO_C->CANrxNew && SDO_C->ptrFunctSignal != NULL) {
            SDO_C->ptrFunctSignal();
        }
    }
}
...
/******************************************************************************/
CO_ReturnError_t CO_SDO_Master::CO_SDOclient_init(
        CO_SDOclient_t         *SDO_C,
        CO_SDO_t               *SDO,
...
/******************************************************************************/
void CO_SDO_Master::CO_SDOclient_initCallback(
        CO_SDOclient_t         *SDOclient,
        void                  (*pFunctSignal)(void))
{
    if(SDOclient != NULL){
        SDOclient->ptrFunctSignal = pFunctSignal;
    }
}


/******************************************************************************/
CO_SDOclient_return_t CO_SDO_Master::CO_SDOclient_setup(
        CO_SDOclient_t         *SDO_C,
        ...
        uint8_t                 nodeIDOfTheSDOServer)
{
    ...
    if(SDO_C->COB_IDClientToServerPrev != idCtoS || SDO_C->COB_IDServerToClientPrev != idStoC) {
        ...
        SDO_C->CANtxBuff = ptrCODriverClass->CAN_Tx_BufferInit(
...
static void CO_SDOclient_abort(CO_SDOclient_t *SDO_C, uint32_t code){
    ...
    SDO_C->CANtxBuff->data[3] = SDO_C->subIndex;
     ptrCODriverClass->CO_memcpySwap4(&SDO_C->CANtxBuff->data[4], &code);
     ptrCODriverClass->CAN_Send(SDO_C->CANtxBuff);
    SDO_C->state = SDO_STATE_NOTDEFINED;
    SDO_C->CANrxNew = false;
}
 ...
 *
 ******************************************************************************/
CO_SDOclient_return_t CO_SDO_Master::CO_SDOclientDownloadInitiate(
        CO_SDOclient_t         *SDO_C,
        uint32_t                index,
        uint8_t                 subIndex,
        uint8_t                *dataTx,
        ...
        uint8_t                 blockEnable)
{
    ...
    if(dataSize <= 4){
        ...
        len = dataSize;
        ptrCODriverClass->CO_memcpySwap4(&SDO_C->CANtxBuff->data[4], &len);
    }
...
/******************************************************************************/
CO_SDOclient_return_t CO_SDO_Master::CO_SDOclientDownload(
        CO_SDOclient_t         *SDO_C,
        ...
        uint32_t               *pSDOabortCode)
{
    ...
    *pSDOabortCode = CO_SDO_AB_NONE;
    ...
    if(SDO_C->SDO && SDO_C->SDOClientPar->nodeIDOfTheSDOServer == SDO_C->SDO->nodeId){
        ...
        /* init ODF_arg */
        *pSDOabortCode =ptrCODriverClass->CO_SDO_initTransfer( SDO_C->index, SDO_C->subIndex);
        ...
        *pSDOabortCode = ptrCODriverClass->CO_SDO_writeOD( SDO_C->bufferSize);
        if((*pSDOabortCode) != CO_SDO_AB_NONE){
            return CO_SDOcli_endedWithServerAbort;
        }
...
/*  RX data ****************************************************************************************** */
    if(SDO_C->CANrxNew){
        ...
        if (SDO_C->CANrxData[0] == (SCS_ABORT<<5)){
            SDO_C->state = SDO_STATE_NOTDEFINED;
            ptrCODriverClass->CO_memcpySwap4(pSDOabortCode , &SDO_C->CANrxData[4]);
            SDO_C->CANrxNew = false;
            return CO_SDOcli_endedWithServerAbort;
        }
...
/*  TX data ******************************************************************************************* */
    ...
    switch (SDO_C->state){
        ...
        case SDO_STATE_BLOCKDOWNLOAD_CRC:{
            ...
            tmp16 = ptrCODriverClass->crc16_ccitt((unsigned char *)SDO_C->buffer, (unsigned int)SDO_C->bufferSize, 0);
 ...
 *
 ******************************************************************************/
CO_SDOclient_return_t CO_SDO_Master::CO_SDOclientUploadInitiate(
        CO_SDOclient_t         *SDO_C,
        uint32_t                index,
        uint8_t                 subIndex,
        uint8_t                *dataRx,
...
/******************************************************************************/
CO_SDOclient_return_t CO_SDO_Master::CO_SDOclientUpload(
        CO_SDOclient_t         *SDO_C,
        uint32_t                timeDifference_ms,
        uint32_t                SDOtimeoutTime,
        uint32_t               *pDataSize,
        uint32_t               *pSDOabortCode)
{
    ...
    *pSDOabortCode = CO_SDO_AB_NONE;
    ...
    if(SDO_C->SDO && SDO_C->SDOClientPar->nodeIDOfTheSDOServer == SDO_C->SDO->nodeId){
        ...
        *pSDOabortCode = ptrCODriverClass->CO_SDO_readOD( SDO_C->bufferSize);
        if((*pSDOabortCode) != CO_SDO_AB_NONE){
            return CO_SDOcli_endedWithServerAbort;
        }
        ...
        switch (SDO_C->state){
            ...
            case SDO_STATE_BLOCKUPLOAD_INITIATE:{
                if (SCS == SCS_UPLOAD_BLOCK){ /*  block upload initiate response */
                    ...
                    if(SDO_C->CANrxData[0]&0x02){
                        uint32_t len;
                        ptrCODriverClass->CO_memcpySwap4(&len, &SDO_C->CANrxData[4]);
                        SDO_C->dataSize = len;
                    }
                    else{
                        SDO_C->dataSize = 0;
            ...
            case SDO_STATE_BLOCKUPLOAD_BLOCK_CRC:{
                if (SCS == SCS_UPLOAD_BLOCK){
                    ...
                    if (SDO_C->crcEnabled){
                        uint32_t tmp16;
                        ptrCODriverClass->CO_memcpySwap2(&tmp16, &SDO_C->CANrxData[1]);

                        if (tmp16 != ptrCODriverClass->crc16_ccitt((unsigned char *)SDO_C->buffer, (unsigned int)SDO_C->dataSizeTransfered, 0)){
...
/******************************************************************************/
void CO_SDO_Master::CO_SDOclientClose(CO_SDOclient_t *SDO_C){
    if(SDO_C != NULL) {
        SDO_C->state = SDO_STATE_NOTDEFINED;
    }
}
