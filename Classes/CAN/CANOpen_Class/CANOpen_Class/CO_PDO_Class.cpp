...
/******************************************************************************/
CO_ReturnError_t CO_PDO_Class::CO_RPDO_init(
        CO_RPDO_t              *RPDO,
        CO_EM_t                *em,
        CO_SDO_t               *SDO,
        CO_SYNC_t              *SYNC,
...
/******************************************************************************/
CO_ReturnError_t CO_PDO_Class::CO_TPDO_init(
        CO_TPDO_t              *TPDO,
        CO_EM_t                *em,
        CO_SDO_t               *SDO,
        uint8_t                *operatingState,
...
uint32_t CO_PDO_Class::CO_TPDOsend(CO_TPDO_t *TPDO){
    ...
    if(TPDO->SDO->ODExtensions){
        ...
        for(i=TPDO->TPDOMapPar->numberOfMappedObjects; i>0; i--){
            uint32_t map = *(pMap++);
            uint32_t index = (uint32_t)(map>>16);
            uint8_t subIndex = (uint8_t)(map>>8);
            uint32_t entryNo = CO_OD_find( index);
            ...
            ODF_arg.attribute = CO_OD_getAttribute( entryNo, subIndex);
            ODF_arg.pFlags = CO_OD_getFlagsPointer( entryNo, subIndex);
        ...
        while(RPDO->CANrxNew[bufNo]){
            ...
            if(RPDO->SDO->ODExtensions){
                ...
                for(i=RPDO->RPDOMapPar->numberOfMappedObjects; i>0; i--){
                    ...
                    ODF_arg.pFlags = CO_OD_getFlagsPointer( entryNo, subIndex);
                    ODF_arg.data = pSDO->OD[entryNo].pData;
                    ODF_arg.dataLength = CO_OD_getLength( entryNo, subIndex);
                    ext->pODFunc(&ODF_arg);
                }
            }
#endif
...
void CO_PDO_Class::Send_TPDO( CO_TPDO_t    *TPDO){
	
	memcpy(&TPDO->CANtxBuff->data[0],&local_rx_buffer.data[0],8);
	TPDO->CANtxBuff->DLC=8;
	 CO_TPDOsend(TPDO);
}