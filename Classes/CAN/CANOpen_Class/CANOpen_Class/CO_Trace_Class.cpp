...
static int32_t getValueI16(void *OD_variable) { return (int32_t) *((uint32_t*)  OD_variable);}
...
static int32_t getValueU16(void *OD_variable) { return (int32_t) *((uint32_t*) OD_variable);}
...
static uint32_t printPointBinary(char *s, uint32_t size, uint32_t timeStamp, int32_t value) {
    if(size < 8) return 0;
    canopen_driver->CO_memcpySwap4(s, &timeStamp);
    canopen_driver->CO_memcpySwap4(s+4, &value);
    return 8;
}
...
static void findVariable(CO_trace_t *trace) {
    ...
    if(!err && (index != 0 || subIndex != 0)) {
        uint32_t entryNo = canopen_driver->CO_OD_find( index);

        if(index >= 0x1000 && entryNo != 0xFFFF && subIndex <= trace->SDO->OD[entryNo].maxSubIndex) {
            OdDataPtr = canopen_driver->CO_OD_getDataPointer( entryNo, subIndex);
        }

        if(OdDataPtr != NULL) {
            uint32_t len = canopen_driver->CO_OD_getLength( entryNo, subIndex);
...
/******************************************************************************/
void CO_Trace_Class::CO_trace_init(
        CO_trace_t             *trace,
        CO_SDO_t               *SDO,
        uint8_t                 enabled,
        uint32_t               *timeBuffer,
        ...
        uint32_t                idx_OD_traceConfig,
        uint32_t                idx_OD_trace)
{
    trace->SDO = SDO;
    ...
    *trace->triggerTime = 0;
    ...
    canopen_driver->CO_OD_configure( idx_OD_traceConfig, CO_ODF_traceConfig, (void*)trace, 0, 0);
    canopen_driver->CO_OD_configure( idx_OD_trace, CO_ODF_trace, (void*)trace, 0, 0);
}


/******************************************************************************/
void CO_Trace_Class::CO_trace_process(CO_trace_t *trace, uint32_t timestamp) {
    if(trace->enabled) {
