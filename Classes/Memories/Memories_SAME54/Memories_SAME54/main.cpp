	...
	while (1) {
		...
		for (int i = 1; i < ISSI_BUFFER_SIZE; i++) {
			if (tx_buffer[i] != rx_buffer[i]) {
				...
				// 				usb<<"bit :"<<i<<NEWLINE;
				i=ISSI_BUFFER_SIZE;
				gpio_set_pin_level(LEDA22,false);
				gpio_set_pin_level(LEDA21,true);
				break;
			}
...
void	FramMemoryTest(void){
	...
	while (1) {
		//delay_us(100);
		/* Write data to flash memory */
		if (ERR_NONE ==fram.WriteAddress((uint8_t *)tx_buffer,m,FRAM_BUFFER_SIZE)) {
			
		}
		delay_ms(WRITE_DELAY);
		//flash.WaitBusy();
		/* Read data from flash memory */
		if (ERR_NONE == fram.ReadAddress((uint8_t *)rx_buffer,m,FRAM_BUFFER_SIZE)) {
			//while(!memory.xferDone);
